...
export default function ProjectCard({id, title, description, creationDate}: Props) {
    return(
        ...
        <div className="items-center px-6 py-4 text-center sm:text-left">
          <p className="text-xl text-white-600">Realizado: {`${creationDate}`}</p>
        </div>
        <Link href={`/${id}`} className="block w-full sm:w-auto px-5 bg-gray-900 text-xl font-bold text-center py-2 rounded-lg hover:bg-lime-500">
          Ver detalles
        </Link>
      </div>
    )
